...
.. tip:: Example
   ...
   The iteration of the backward Euler :eq:`eqn::heatequationBWE` is made by [`:::freefem for` loop](/reference/Loops/#for).
...
**Problem Ev\ :math:`(f,\Omega)`**: For a given :math:`f\in L^2(0,T;V'),\, u^0\in H`
...
Let :math:`\{\mathcal{T}_h\}_{h\downarrow 0}` be regular triangulations (see `Regular Triangulation </documentation/MeshGeneration/#regular-triangulation-htriangle>`__).
...
.. tip:: Convect
   ...
   .. note:: The scheme `:::freefem convect` is unconditionally stable, then the bell become lower and lower (the maximum of $u^{37}$ is $0.406$ as shown in [Fig. 21](#Fig21)).
